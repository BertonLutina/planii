...
export type MLang = 'fr' | 'en' | 'nl' | 'es' | 'pt' | 'it' | 'el' | 'ru' | 'sw'
export const M_LANGS: MLang[] = ['fr', 'en', 'nl', 'es', 'pt', 'it', 'el', 'ru', 'sw']
...
export function mlang(l?: string | null): MLang {
  const v = (l || 'fr').slice(0, 2).toLowerCase()
  return (M_LANGS.includes(v as MLang) ? v : 'fr') as MLang
}

/** Traduit une clé mail dans la langue donnée, avec variables {x}. */
export function mt(lang: string | null | undefined, key: string, vars?: Record<string, string | number>): string {
